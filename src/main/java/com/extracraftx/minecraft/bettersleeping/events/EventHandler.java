...
public class EventHandler{
	...
	public static void onWakeup(PlayerEntity player, boolean b1, boolean b2){
		...
		if (sp.getServerWorld().getTimeOfDay() % 24000L >= 12542L && sp.getServerWorld().getTimeOfDay() % 24000L < 23460L) {
	...
	private static void sendAsleepMessage(List<? extends PlayerEntity> players, long asleep, int total, int percentRequired){
		...
		args.put("required", NumberFormat.getInstance().format((int) ((percentRequired*players.size()/100.0)+0.5)));
